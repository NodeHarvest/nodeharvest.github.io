...
扫描是第二、第三层的基础，因此不能把 provider 的一次故障直接传播成全链路不可用或错误推荐。下一批实现将补齐：
...
5. 指数退避和显式重试，避免重复下载；
6. provider 版本切换与上一版本回滚；
7. macOS、Linux、Windows 的真实平台矩阵验证；
8. 自动安装时同步落盘第三方许可证。

降级原则是：可以使用明确标注为过期的最后成功结果，但不能静默把旧数据当作新扫描结果；没有可信快照时，上层必须得到明确的不可用状态。

## 八、MVP 执行顺序

### 阶段 1：可靠、无感的 Scan 基座

完成 provider 状态机、缓存与过期标识、离线/损坏/版本不兼容降级、回滚、多平台验证和许可证落盘。
...
当前工作位于 `feat/cli-harnesskit-foundation` 分支，已形成以下可追溯提交：
